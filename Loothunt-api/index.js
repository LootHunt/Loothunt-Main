...
app.post('/startgame',async (req,res) => {
    ...
    games.winner = ""
    games.tokenid = req.body.tokenid
...
app.post("/claimprize",auth,async (req,res) => {
    console.log('here')
    let games = await game.findOne({"gameid":req.body.gameid});
    let participate = await student.findOne({address:req.user.address});
    
    if(games){
        if(participate.winner){
            participate.winner.indexOf(req.body.gameid) === -1 ? participate.winner.push(req.body.gameid) : console.log("This item already exists");

        
        }
    games.winner = req.body.address
...
app.get("/wondata",auth,async (req,res) =>{
    let participate = await student.findOne({address:req.user.address});
    let games = await game.find({"gameid":{ $in: participate.winner}});
    res.json({data:games})
